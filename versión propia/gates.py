...
def Exp_mod(c, regx, reg1, regX, reg2, regN, regN_ctrl, ancil_ctrl):
    ...
    """
    Function: 
    It performs modular exponentiation a^x mod N.
    
    Args:
    c: circuit
    regx: binary representation of the exponent x
    reg1: binary representation of the base a
    regX: ancillary register for storing intermediate values of a^(2^i)
    reg2: register where the final result a^x mod N will be stored
    regN: binary representation of the modulus N
    regN_ctrl: ancillary register for modular operations
    ancil_ctrl: ancillary qubits for addition and multiplication

    Return:
    The circuit with the state of reg2 being a^x mod N.
    """
    n = len(regx)
    if n != len(reg1) or n != len(regX) or n != len(reg2):
        raise Exception("All registers must be of the same length")
    
    # Initialize reg2 to 1, as any number to the power of 0 is 1.
    c.x(reg2[0])

    # Compute a^(2^i) for each bit of the exponent
    for i in range(n):
        # Copy a into regX
        for j in range(n):
            c.cx(reg1[j], regX[j])
        
        # Perform modular multiplication a^(2^i) mod N
        for _ in range(i):
            c = Mult_mod(c, regX, reg1, regX[0], regX, regN, regN_ctrl, ancil_ctrl)

        # If the i-th bit of the exponent is 1, multiply the result by a^(2^i)
        c.ccx(regx[i], regX[0], ancil_ctrl[0])  # Control on the i-th bit of regx and the LSB of regX
        c = Mult_mod(c, regX, reg1, ancil_ctrl[0], reg2, regN, regN_ctrl, ancil_ctrl, reverse=False)
        c.ccx(regx[i], regX[0], ancil_ctrl[0])
